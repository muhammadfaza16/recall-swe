  ...
  topics: [
    {
      ...
      content_casual: 'Lo mungkin bisa bikin fitur A-Z pakai framework favorit lo, tapi kalau lo nggak ngerti hardware yang nge-run kode lo, lo bakal buta pas nge-solve performance issue. Mari kita telanjangi mesinnya.\n\n**CPU & Registers:** CPU itu koki super ngebut. Pas dia masak (eksekusi kode), dia taruh bumbu di meja terdekat (*Registers*). Kalau mejanya penuh, dia nyari di rak (*Cache*), baru ke gudang (*RAM*). Terakhir, baru ke pasar (*SSD*).\n\n**Memory Hierarchy (Rahasia Kecepatan):** L1 Cache itu kilat (~1ns). RAM? ~100ns (100x lebih lambat bro!). Hardisk? Ribuan kali lipat. Kode lo lambat biasanya bukan karena algoritmanya jelek, tapi karena CPU lo kebanyakan nungguin data dari RAM (Cache Miss).\n\n**Spatial Locality:** CPU itu pinter, dia males ngambil barang satu-satu. Kalau lo minta data di index 0, dia sekalian ngambil index 1 sampai 15 ke dalem *Cache Line*. Ini kenapa nge-loop *Array* (yang datanya nempel) bakal membantai performa nge-loop *Linked List* (yang datanya mencar-mencar) meskipun sama-sama O(n)!',
      why_casual: 'Kalau lo mau tembus FAANG atau nanganin sistem high-load, lo nggak bisa cuma nebak. Optimasi tanpa paham *Cache Locality* itu kayak dukun. Engineer top tahu kapan harus pakai Array sederhana ketimbang Tree kompleks cuma demi bikin CPU seneng (cache-friendly).',
      mistake_casual: 'Terlalu mendewa-dewakan Big O Notation. Lo pikir Hash Map O(1) selalu lebih ngebut dari Array O(n)? Di dunia nyata, overhead *hashing* dan loncat-loncat di RAM bikin Hash Map sering keok ngelawan *brute-force* iterasi Array pada jumlah data kecil (misal n < 100).',
      interview: [
        {
    ...
    },
    {
      ...
      content_casual: 'Ngoding itu nggak cuma asal muter otak sampai fitur jalan. Kalau codebase lo udah segede raksasa, lo butuh "Aliran Bela Diri" yang jelas biar nggak bunuh diri pas *maintenance*. Ada dua aliran raksasa:\n\n**Object-Oriented Programming (OOP):** Pendekatan "Real World". Lo gabungin Data (State) sama Action (Behavior) di dalem satu objek. Tujuannya? Biar orang luar nggak seenaknya ngacak-ngacak data lo (Encapsulation). Cocok banget buat modelin sistem enterprise yang kompleks kayak e-commerce atau game.\n\n**Functional Programming (FP):** Aliran anti-ribet. Prinsipnya simpel: Data dan Fungsi itu musuhan, jangan digabung. Lo panggil fungsi A, hasilnya B. Nggak bakal ada "variabel global" yang mendadak berubah di belakang layar (*Pure Functions & Immutability*). Bikin UI (kayak React) atau stream data jadi gampang banget di-test dan anti bug aneh-aneh.',
      why_casual: 'Engineer yang cuma mentok di satu paradigma itu ibarat tukang kayu yang cuma punya palu; semua masalah dianggap paku. Paham OOP bikin lo jago nulis arsitektur Backend yang modular. Paham FP bikin lo dewa nulis React/UI yang state-nya predictable dan nggak gampang bocor.',
      mistake_casual: 'Di FP: Nge-push data langsung ke array yang lagi dipakai React (`arr.push(x)`), terus bingung kenapa UI-nya nggak re-render. Di OOP: Bikin *Inheritance* (pewarisan class) sampai 7 turunan ke bawah (God Object), pas ada bug di parent, semua class anaknya ikut hancur. Selalu pilih *Composition over Inheritance*!',
      interview: [
        {
...
**Struktur Data Krusial:** Array adalah blok memori berurutan — akses by index O(1), insert di tengah O(n). HashMap menggunakan fungsi hash untuk memetakan key ke "bucket" memori. Saat HashMap perlu resize (biasanya saat 75% penuh), ia mengalokasikan memori baru dan re-hash semua entry — O(n) sesekali, tapi amortized O(1). Stack (LIFO) dan Queue (FIFO) adalah abstraksi penting — Stack digunakan call stack itu sendiri, Queue digunakan Event Loop.`,
      ...
      content_casual: `Big O Notation adalah cara kita berbicara tentang performa sebuah algoritma dalam skala besar. Ini bukan tentang berapa detik kode lo berjalan di laptop lo — ini tentang bagaimana runtime dan penggunaan memori bertumbuh seiring bertambahnya jumlah input (n).
...
**Struktur Data Krusial:** Array adalah blok memori berurutan — akses by index O(1), insert di tengah O(n). HashMap menggunakan fungsi hash untuk memetakan key ke "bucket" memori. Saat HashMap perlu resize (biasanya saat 75% penuh), ia mengalokasikan memori baru dan re-hash semua entry — O(n) sesekali, tapi amortized O(1). Stack (LIFO) dan Queue (FIFO) adalah abstraksi penting — Stack digunakan call stack itu sendiri, Queue digunakan Event Loop.`,
      why_casual: `Di interview, soal algoritma hampir selalu tentang mengoptimalkan dari O(n²) ke O(n) menggunakan HashMap. Di production, O(n²) yang terselip dalam kode yang dieksekusi setiap HTTP request adalah resep server yang lambat dan tagihan cloud mahal. Senior engineer membaca kode dan secara insting mengenali bottleneck kompleksitas sebelum menyentuh production.`,
      mistake_casual: `Menggunakan Array.includes() atau Array.find() di dalam sebuah loop — menghasilkan O(n²) tanpa sadar. Solusinya: masukkan data ke Set atau Map lebih dulu, lalu lakukan lookup. Juga: tidak mempertimbangkan Space Complexity. Kode O(n) time bisa tetap berbahaya kalau ia membuat O(n) copies data di memori.`,
      interview: [
        {
...
**Event Loop — Mekanisme Sebenarnya:** Runtime memiliki: Call Stack (eksekusi kode synchronous), Web APIs / libuv (timer, fetch, file I/O — dijalankan di luar stack), Task Queue / Macrotask (callback dari setTimeout, setInterval, I/O), dan Microtask Queue (callback dari Promise.then, queueMicrotask). Urutan: (1) Jalankan semua di Call Stack sampai kosong. (2) Drain seluruh Microtask Queue. (3) Ambil SATU task dari Macrotask Queue. (4) Ulangi dari langkah 2.`,
      ...
      content_casual: `JavaScript tidak hanya dieksekusi langsung — ia diproses oleh engine (V8 di Node.js dan Chrome). V8 melakukan Just-In-Time (JIT) compilation: fungsi yang sering dipanggil (hot functions) dioptimasi oleh compiler Turbofan menjadi machine code yang sangat cepat. Tapi jika lo tiba-tiba mengubah tipe data sebuah parameter (fungsi yang biasanya menerima Number, tiba-tiba menerima String), V8 harus "deoptimize" — membuang machine code yang sudah dikompilasi dan kembali ke interpretasi.
...
**Event Loop — Mekanisme Sebenarnya:** Runtime memiliki: Call Stack (eksekusi kode synchronous), Web APIs / libuv (timer, fetch, file I/O — dijalankan di luar stack), Task Queue / Macrotask (callback dari setTimeout, setInterval, I/O), dan Microtask Queue (callback dari Promise.then, queueMicrotask). Urutan: (1) Jalankan semua di Call Stack sampai kosong. (2) Drain seluruh Microtask Queue. (3) Ambil SATU task dari Macrotask Queue. (4) Ulangi dari langkah 2.`,
      why_casual: `Pemahaman Closure adalah fondasi React Hooks. Setiap custom hook menggunakan closure untuk "mengingat" state. Stale closure adalah sumber bug paling membingungkan di React. Pemahaman Event Loop adalah kunci kenapa operasi tertentu "tidak menunggu" dan kenapa race condition bisa terjadi di async code.`,
      mistake_casual: `Menggunakan var di dalam loop (function-scoped, bukan block-scoped): semua callback dalam loop mereferensikan variabel yang sama (nilai akhirnya, bukan nilai saat loop berjalan). Gunakan let atau const. Juga: melakukan loop dengan forEach() + async/await — forEach tidak menunggu Promise, sehingga semua iterasi berjalan paralel tanpa kendali.`,
      interview: [
        {
...
**Type Narrowing:** Setelah melewati conditional (if, instanceof, typeof, discriminant check), TypeScript mempersempit tipe secara otomatis di dalam scope tersebut. Ini membuat kode lebih safe sekaligus lebih ekspresif — tidak perlu type assertion.`,
      ...
      content_casual: `TypeScript bukan sekedar "JavaScript dengan type annotation". Pada level production, TypeScript adalah alat desain arsitektur — cara mendefinisikan kontrak antar bagian sistem sehingga compiler menjadi garda terdepan yang mencegah bug sebelum kode dijalankan.
...
**Type Narrowing:** Setelah melewati conditional (if, instanceof, typeof, discriminant check), TypeScript mempersempit tipe secara otomatis di dalam scope tersebut. Ini membuat kode lebih safe sekaligus lebih ekspresif — tidak perlu type assertion.`,
      why_casual: `TypeScript yang digunakan dengan benar membuat refactoring besar aman (compiler berteriak di mana pun kontrak dilanggar), membuat onboarding developer baru lebih cepat (tipe adalah dokumentasi yang tidak pernah outdated), dan mengeliminasi seluruh kategori bug runtime (null/undefined access, salah tipe parameter).`,
      mistake_casual: `Menggunakan 'as' (type assertion) atau 'any' sebagai jalan pintas. 'as' mengatakan ke compiler "percayai aku" — ini mematikan type checking dan sama saja seperti tidak menggunakan TypeScript. Bug dari type assertion tidak tertangkap sampai runtime. Gunakan type guard (is operator) atau discriminated union sebagai gantinya.`,
      interview: [
        {
...
**Git Bisect:** Binary search di history commit. Tandai commit good dan bad, git otomatis checkout commit tengah untuk di-test. Menemukan commit penyebab bug dalam O(log n) langkah.`,
      ...
      content_casual: `Git bukan sekedar "backup kode". Git adalah content-addressable filesystem. Memahami cara Git menyimpan data secara internal menghilangkan rasa takut saat menghadapi conflict atau mencari bug.
...
**Git Bisect:** Binary search di history commit. Tandai commit good dan bad, git otomatis checkout commit tengah untuk di-test. Menemukan commit penyebab bug dalam O(log n) langkah.`,
      why_casual: `Kemampuan Git yang solid langsung terlihat di tim profesional. Engineer yang paham rebase bisa menjaga history bersih dan PR mudah di-review. Engineer yang bisa git bisect menemukan penyebab bug regression dalam menit, bukan jam. Dan engineer yang tidak sengaja force-push ke main bisa menghancurkan kerja tim seharian.`,
      mistake_casual: `Force pushing (git push -f) ke branch shared. Commit pesan yang tidak informatif ('fix', 'update', 'wip'). Mengerjakan banyak perubahan tidak berhubungan dalam satu commit — membuat git revert atau debug mustahil. Tidak menggunakan 'git stash' saat perlu switch context, sehingga commit "work in progress" mengotori history.`,
      interview: [
        {
