...
**Go GC: Tri-Color Mark-Sweep:** Semua objek dimulai sebagai "white". GC dimulai dari roots (stack, globals) dan menandai objek yang bisa dijangkau sebagai "grey" lalu "black". Objek yang tetap "white" adalah garbage. Go GC berjalan concurrent (tidak fully stop-the-world) dengan target latency 1-2ms pause time.`,
      ...
      content_casual: `Setiap program mendapat dua wilayah memori utama dari OS: Stack dan Heap. Memahami perbedaannya secara mendalam adalah dasar dari debugging memory leak dan performance tuning di production.
...
**Go GC: Tri-Color Mark-Sweep:** Semua objek dimulai sebagai "white". GC dimulai dari roots (stack, globals) dan menandai objek yang bisa dijangkau sebagai "grey" lalu "black". Objek yang tetap "white" adalah garbage. Go GC berjalan concurrent (tidak fully stop-the-world) dengan target latency 1-2ms pause time.`,
      why_casual: `Di production: memory leak menyebabkan server crash setelah berjam-jam running. Di Go, struct kecil yang di-pass by value (bukan pointer) menghindari Heap allocation dan mengurangi GC pressure — langsung berdampak pada latency. Di Node.js, menyimpan data besar di closure atau module-level variable mengisi Old Generation dan menyebabkan Major GC yang membuat server lag.`,
      mistake_casual: `Di Go: reflex me-return pointer dari setiap fungsi dengan asumsi "menghindari copy lebih efisien". Untuk struct kecil (< ~128 bytes), copy by value di Stack jauh lebih cepat dari Heap allocation + GC overhead. Di Node.js: tidak pernah menghapus event listener. Setiap emitter.on(event, handler) yang tidak di-off() menyimpan referensi ke handler (closure), yang bisa menyimpan referensi ke objek besar — classic Node.js memory leak.`,
      interview: [
        {
...
**Python GIL (Global Interpreter Lock):** CPython hanya bisa mengeksekusi satu thread Python pada satu waktu, bahkan di multi-core. Threading Python hanya berguna untuk I/O-bound (GIL dilepas saat menunggu I/O). Untuk CPU-bound: gunakan multiprocessing (separate processes) atau library yang melepaskan GIL (numpy, pandas menggunakan C extensions).`,
      ...
      content_casual: `Concurrency adalah tentang menangani banyak hal sekaligus (struktur). Parallelism adalah tentang mengerjakan banyak hal secara harfiah bersamaan (eksekusi). Lo bisa punya concurrent program yang berjalan di single-core (tidak parallel), dan parallel program yang tidak concurrent.
...
**Python GIL (Global Interpreter Lock):** CPython hanya bisa mengeksekusi satu thread Python pada satu waktu, bahkan di multi-core. Threading Python hanya berguna untuk I/O-bound (GIL dilepas saat menunggu I/O). Untuk CPU-bound: gunakan multiprocessing (separate processes) atau library yang melepaskan GIL (numpy, pandas menggunakan C extensions).`,
      why_casual: `Ini adalah fondasi dari setiap keputusan arsitektur backend. Apakah butuh message queue? Worker pool? Kenapa Next.js lebih cepat dari Node.js murni untuk rendering? Semua bergantung pada pemahaman concurrency model. Senior engineer tahu kapan Node.js cukup, kapan harus pakai Go, dan kapan harus pindah ke message queue untuk async processing.`,
      mistake_casual: `Menjalankan bcrypt.hash() (CPU-intensive, ~100-300ms) di dalam Express request handler tanpa async version. Ini memblokir seluruh event loop. Di Go: spawn goroutine tanpa batas di dalam HTTP handler — saat traffic spike, ribuan goroutine menyebabkan memory exhaustion.`,
      interview: [
        {
...
**libuv Thread Pool:** Node.js menggunakan epoll/kqueue untuk network I/O (sangat cepat). Untuk file system I/O, Node.js menggunakan thread pool (default 4 threads) karena kebanyakan OS tidak mendukung async file I/O dengan baik.`,
      ...
      content_casual: `I/O adalah operasi yang paling sering dilakukan server backend: baca dari database, tulis ke disk, kirim HTTP ke service lain. Ada empat model I/O yang berbeda secara fundamental.
...
**libuv Thread Pool:** Node.js menggunakan epoll/kqueue untuk network I/O (sangat cepat). Untuk file system I/O, Node.js menggunakan thread pool (default 4 threads) karena kebanyakan OS tidak mendukung async file I/O dengan baik.`,
      why_casual: `Saat debugging mengapa server lambat di traffic tinggi, lo perlu tahu model I/O mana yang digunakan. Apakah bottleneck di epoll (network) atau thread pool (file I/O)? Ini menentukan apakah solusinya horizontal scaling, menambah thread pool size (UV_THREADPOOL_SIZE), atau menggunakan streaming.`,
      mistake_casual: `Menggunakan fs.readFileSync() atau fs.writeFileSync() di production Node.js handler. Ini menggunakan blocking syscall LANGSUNG di main thread — bukan melalui thread pool — memblokir seluruh event loop. Ini bug serius. Selalu gunakan fs.promises.readFile() atau fs.createReadStream().`,
      interview: [
        {
