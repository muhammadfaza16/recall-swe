  ...
  topics: [
    {
      ...
      content_casual: 'Bikin backend itu bukan cuma asal narik data dari DB terus di-return jadi JSON. Itu sih level tutorial. Di *Real World*, ini tentang gimana kode lo bisa bertahan 5 tahun tanpa bikin engineer penerus lo resign.\n\n**Layered Architecture (Router-Controller-Service-Repository):**\n- **Router:** Satpam depan. Cek tiket (URL & Method), terus oper ke Controller.\n- **Controller:** Tukang validasi. Cek apakah *body* request-nya bener (misal pake Zod). Abis itu oper ke Service.\n- **Service (Business Logic):** Ini otak emas aplikasi lo. Cara ngitung pajak, ngasih diskon, dll ada di sini. **Aturan mutlak:** Service nggak boleh tau urusan HTTP (`req`/`res`) atau urusan SQL!\n- **Repository:** Kuli gudang. Cuma dia yang boleh ngobrol sama Database (ORM/SQL).\n\n**Kenapa Ribet Banget?** Bayangin besok bos lo minta ganti dari REST API ke GraphQL. Kalau lo pake *layering*, lo **CUMA** ganti Controller/Router-nya aja. Logika bisnis lo di Service tetep utuh, nggak perlu disentuh!',
      why_casual: 'Tanpa *layering*, lo bakal ngelahirin *God Controller* — satu fungsi 1000 baris yang ngurusin HTTP, nembak DB, ngitung diskon, sampai ngirim email. Kode kayak gini mustahil di-*Unit Test* dan bakal hancur lebur pas di-scale.',
      mistake_casual: 'Nulis query DB (`prisma.user.find()`) langsung di dalem Controller. Pas lo mau jalanin query yang sama lewat *Cron Job* di *background*, lo nangis karena *Cron Job* nggak punya *object* HTTP Request/Response!',
      interview: [
        {
    ...
    },
    {
      ...
      content_casual: 'Middleware itu kayak *Security Check* di bandara. Lo mau masuk pesawat (Controller)? Lewatin metal detector dulu!\n\n**The Onion Model:** Bayangin request masuk nembus lapisan-lapisan bawang. Kalau aman, dia nyampe ke inti (Controller). Pas balikin respons, dia nembus lapisan bawang yang sama dari dalem ke luar.\n\n**Paling Sering Dipake Buat Apa?**\n1. **Auth/Login:** Middleware ngecek JWT Token lo. Kalau Token basi, dia langsung nendang lo pake status `401 Unauthorized`. Controller utama bahkan nggak sadar lo pernah nyoba masuk.\n2. **Logging:** Nyatet jam berapa *request* masuk, manggil `next()`, nungguin Controller kelar, terus nyatet jam berapa respons keluar (buat ngukur performa API).\n3. **Global Error Handling:** Nangkep error dari mana aja dan ngebungkus error-nya jadi JSON yang rapi biar Frontend nggak nerima error HTML jelek.\n\nDi Express/Koa, nyawa middleware itu ada di fungsi `next()`. Lupa manggil `next()`? *Request* lo bakal nyangkut di *loading screen* sampai kiamat (*Timeout*).',
      why_casual: 'Middleware misahin urusan teknis dari logika bisnis. Daripada lo nulis kode "Cek apakah user udah login" di 50 *Controller* yang beda, lo cukup pasang 1 baris Middleware di depan *Router*. Kode lo jadi super bersih.',
      mistake_casual: 'Paling sering: Lupa naruh `return` setelah `res.status(400).send()`. Akhirnya kode tetep jalan ke bawah dan manggil `next()`. Muncul deh error legendaris Node.js: *"Cannot set headers after they are sent to the client"*!',
      interview: [
        {
...
**Cluster vs Worker Threads:** Cluster: multiple Node.js processes (shared port via IPC, memory terpisah, crash satu tidak pengaruhi lain). Worker Threads: multiple threads dalam satu process (shared memory via SharedArrayBuffer, V8 isolate sendiri). Cluster untuk scaling I/O-bound server, Worker Threads untuk CPU-bound tasks.`,
      ...
      content_casual: `Node.js menggabungkan V8 (JavaScript engine), libuv (async I/O library), dan built-in modules. Memahami arsitektur ini menjelaskan kenapa Node.js sangat baik untuk I/O-bound work dan sangat buruk untuk CPU-bound work.
...
**Cluster vs Worker Threads:** Cluster: multiple Node.js processes (shared port via IPC, memory terpisah, crash satu tidak pengaruhi lain). Worker Threads: multiple threads dalam satu process (shared memory via SharedArrayBuffer, V8 isolate sendiri). Cluster untuk scaling I/O-bound server, Worker Threads untuk CPU-bound tasks.`,
      why_casual: `Memahami event loop phases menjelaskan urutan eksekusi yang tidak intuitif. Thread pool yang tersumbat adalah silent bottleneck — file I/O tiba-tiba 10x lebih lambat tanpa pesan error apapun. Diagnosis: monitor lag event loop dengan perf_hooks atau clinic.js.`,
      mistake_casual: `process.nextTick() rekursif bisa mem-starve seluruh event loop karena nextTick dieksekusi sebelum pindah ke phase apapun — tidak pernah memberi I/O callbacks kesempatan jalan. fs.readFileSync() di production handler: ini blocking syscall LANGSUNG di main thread, bukan melalui thread pool.`,
      interview: [
        {
...
**Rate Limiting:** Sliding Window dengan Redis lebih presisi dari Fixed Window. Implementasi: Redis sorted set atau lua script atomic. Ekspos headers X-RateLimit-Limit, X-RateLimit-Remaining, Retry-After.`,
      ...
      content_casual: `Arsitektur API adalah keputusan fundamental. REST untuk public/multi-client, GraphQL untuk BFF (Backend for Frontend) dengan banyak clients, tRPC untuk full-stack TypeScript monorepo.
...
**Rate Limiting:** Sliding Window dengan Redis lebih presisi dari Fixed Window. Implementasi: Redis sorted set atau lua script atomic. Ekspos headers X-RateLimit-Limit, X-RateLimit-Remaining, Retry-After.`,
      why_casual: `API yang tidak divalidasi menyebabkan kerentanan security. Tidak ada rate limiting = rentan DoS. Pilihan arsitektur yang salah di awal mahal untuk di-refactor ketika tim dan clients sudah banyak.`,
      mistake_casual: `Return 200 OK untuk semua response termasuk error dengan { status: "error" } di body — mematikan semua monitoring berbasis HTTP status. Tidak memvalidasi tipe dan range data (angka negatif sebagai quantity, string panjang sebagai nama). Tidak membatasi ukuran request body — attacker bisa kirim payload 10GB.`,
      interview: [
        {
...
**Test Doubles:** Stub (return nilai tetap), Mock (verify pemanggilan dengan argumen tertentu), Spy (wrap fungsi asli dan catat info call), Fake (implementasi sederhana — in-memory DB). Jangan over-mock: jika mock segalanya, test hanya memverifikasi mock dipanggil, bukan bahwa sistem benar.`,
      ...
      content_casual: `Testing adalah tentang membangun confidence bahwa perubahan tidak merusak hal yang tidak terduga, bukan tentang mencapai 100% coverage.
...
**Test Doubles:** Stub (return nilai tetap), Mock (verify pemanggilan dengan argumen tertentu), Spy (wrap fungsi asli dan catat info call), Fake (implementasi sederhana — in-memory DB). Jangan over-mock: jika mock segalanya, test hanya memverifikasi mock dipanggil, bukan bahwa sistem benar.`,
      why_casual: `Test suite yang solid adalah safety net untuk refactoring. Tanpa test, developer takut mengubah kode lama. Dengan test, CI/CD memberitahu jika ada regresi. Test adalah dokumentasi executable yang tidak pernah outdated.`,
      mistake_casual: `Testing implementation detail (bahwa fungsi internal X dipanggil) alih-alih behavior (output yang benar). Test seperti ini brittle — rusak setiap kali refactor internals meskipun behavior tidak berubah. Test harus sebagai black-box dari perspektif user.`,
      interview: [
        {
