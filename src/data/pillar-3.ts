  ...
  topics: [
    {
      ...
      content_casual: 'Bikin query se-dewa apapun nggak bakal nyelamatin lo kalau desain tabel database lo udah ngaco dari awal.\n\n**ERD & Relasi:** Lo harus ngerti cara ngegambarin hubungan data. Ada *1-to-Many* (1 User punya banyak Postingan) dan *Many-to-Many* (1 User punya banyak Role, 1 Role dipake banyak User — lo butuh *Junction Table* di sini).\n\n**Normalization:** Ini cara elegan buat mencegah data berulang (duplikat).\n- **1NF:** Jangan pernah nyimpen *array* atau koma-komaan di dalem satu kolom string!\n- **3NF:** Intinya, kalau kolom B itu sebenernya ngejelasin kolom A, bukan ngejelasin ID utama tabel itu, pisahin jadi tabel baru!\n\n**ORM & The N+1 Problem:** ORM (Prisma/TypeORM) itu enak, tapi mematikan kalau lo buta. Lo ambil 10 User (1 query), terus lo *looping* 10 user itu buat ngambil `user.getOrders()`. ORM lo bakal nembak DB 10 kali lagi di dalem loop. Total 11 Query! Server lo bakal teriak. Solusinya? *Eager Loading* (pake `JOIN` dari awal).',
      why_casual: 'Desain DB yang busuk di awal bakal jadi *tech debt* permanen yang ngebunuh startup lo 2 tahun kemudian. Ngerti normalisasi itu ngebentengi sistem lo dari data anomali (nama user diubah di profil, tapi di invoice masih nama lama).',
      mistake_casual: 'Mendewakan 3NF membabi-buta. Di skala *Enterprise* kayak Tokopedia/Gojek, kebanyakan `JOIN` itu ngebantai CPU database. Arsitek top sering ngelakuin *Denormalization* secara sengaja: nyimpen sedikit data duplikat biar kalau di-SELECT super ngebut tanpa perlu nge-JOIN 5 tabel.',
      interview: [
        {
    ...
    },
    {
      ...
      content_casual: 'Milih database itu kayak milih senjata buat perang. Nggak ada satu senjata sakti buat semua masalah.\n\n**SQL (PostgreSQL, MySQL):**\nSenjata berat dengan aturan super ketat (*strict schema*). Bagus buat data yang bentuknya jelas dan butuh **Konsistensi Tinggi** (ACID). Relasi antar tabel diikat kuat pakai *Foreign Keys*. *Use case: Sistem Bank, Inventory, ERP.*\n\n**NoSQL - Document (MongoDB):**\nSi fleksibel nan dinamis (*schema-less*). Nyimpen data pake format mirip JSON. Lo bisa nambahin kolom baru kapan aja tanpa pusingin row lain. Tapi ingat, NoSQL itu *lemah banget di operasi JOIN*. *Use case: Catalog E-commerce, CMS, atau fitur yang tiap minggu dirombak PM.*\n\n**NoSQL - Key-Value (Redis):**\nCuma butuh "Kunci" dan "Nilai". Berjalan di RAM, makanya super ngebut kayak kilat. Tapi lo nggak bisa query aneh-aneh (misal nyari data yang harganya > 100). *Use case: Caching, User Session, Rate Limiting.*',
      why_casual: 'Banyak dev junior pakai MongoDB murni karena males belajar SQL dan males nulis *Schema Migration*. Pas sistemnya makin gede dan datanya butuh relasi rumit, kode backend-nya jadi kusut karena harus nge-JOIN manual di memori JS. Milih DB = milih masa depan.',
      mistake_casual: 'Maksa pakai SQL (Postgres) buat nyimpen data log klik user yang jumlahnya bisa puluhan juta per hari. Proses `INSERT` di SQL itu mahal karena harus update *index*. Buat data gila-gilaan kayak gitu, mending buang ke NoSQL Wide-Column (Cassandra) atau Time-Series Database.',
      interview: [
        {
...
**EXPLAIN ANALYZE:** Jalankan sebelum query slow di production. Tunjukkan: tipe operasi (Seq Scan vs Index Scan), estimated vs actual rows (jauh berbeda = stale statistics, perlu ANALYZE), dan cost. "Seq Scan" pada tabel besar hampir selalu berarti missing index.`,
      ...
      content_casual: `SQL adalah bahasa yang paling underestimated. Banyak yang tahu SELECT dasar, tapi SQL yang benar-benar powerful ada di Window Functions, CTEs, dan kemampuan membaca query execution plan.
...
**EXPLAIN ANALYZE:** Jalankan sebelum query slow di production. Tunjukkan: tipe operasi (Seq Scan vs Index Scan), estimated vs actual rows (jauh berbeda = stale statistics, perlu ANALYZE), dan cost. "Seq Scan" pada tabel besar hampir selalu berarti missing index.`,
      why_casual: `Query yang tidak dioptimasi adalah penyebab paling umum API lambat di production. Menambah satu composite index yang tepat bisa mengubah query dari 30 detik menjadi 30 milidetik — tanpa mengubah satu baris kode aplikasi.`,
      mistake_casual: `SELECT * di production code. Selain mengambil data tidak dibutuhkan, ia mencegah Index Only Scan — database terpaksa fetch heap untuk kolom yang tidak di-index. N+1 query: fetch list of users, lalu untuk setiap user fetch profile-nya secara terpisah. Gunakan JOIN atau eager loading.`,
      interview: [
        {
...
**Covering Index (Index Only Scan):** Jika query hanya butuh kolom yang semua ada di index, database tidak perlu mengakses heap — "Index Only Scan". Di PostgreSQL, gunakan INCLUDE clause untuk menambahkan kolom non-key ke leaf nodes.`,
      ...
      content_casual: `Index adalah struktur data terpisah yang database maintain untuk mempercepat query. Default index type di PostgreSQL dan MySQL adalah B-Tree (Balanced Tree).
...
**Covering Index (Index Only Scan):** Jika query hanya butuh kolom yang semua ada di index, database tidak perlu mengakses heap — "Index Only Scan". Di PostgreSQL, gunakan INCLUDE clause untuk menambahkan kolom non-key ke leaf nodes.`,
      why_casual: `Indexing adalah lever performa dengan ROI tertinggi di backend engineering. Tapi over-indexing memperlambat write operations. Senior engineer melakukan indexing berdasarkan actual query patterns dari APM/slow query log, bukan spekulasi.`,
      mistake_casual: `Membuat index untuk setiap kolom secara terpisah. Tidak menghapus index yang tidak digunakan (cek pg_stat_user_indexes). Index bloat setelah banyak UPDATE/DELETE — REINDEX CONCURRENTLY diperlukan. Menggunakan LIKE '%keyword%' yang tidak bisa menggunakan B-Tree index — gunakan Full-Text Search atau pg_trgm.`,
      interview: [
        {
...
**Cache Stampede (Thundering Herd):** Saat cache key expired dan ribuan request datang bersamaan, semua menghantam database. Solusi: Probabilistic Early Expiration (refresh sebelum expired), Redis SETNX lock (hanya satu worker yang refresh), atau singleflight pattern.`,
      ...
      content_casual: `Caching adalah seni menyimpan hasil komputasi mahal di tempat yang lebih cepat diakses, dengan trade-off bahwa data mungkin stale.
...
**Cache Stampede (Thundering Herd):** Saat cache key expired dan ribuan request datang bersamaan, semua menghantam database. Solusi: Probabilistic Early Expiration (refresh sebelum expired), Redis SETNX lock (hanya satu worker yang refresh), atau singleflight pattern.`,
      why_casual: `Tanpa caching, database menjadi bottleneck saat traffic spike. Dengan caching tepat, bisa serve 100x lebih banyak request dengan hardware yang sama. Tapi caching menambah kompleksitas — stale data, invalidation bugs, stampede adalah masalah nyata yang harus diantisipasi sejak desain.`,
      mistake_casual: `Caching tanpa TTL — data stale selamanya. Meng-cache granularity yang salah (seluruh dashboard page padahal hanya sebagian data yang sering berubah). Menggunakan Redis sebagai primary database untuk data yang harus persistent tanpa mengkonfigurasi RDB/AOF persistence.`,
      interview: [
        {
...
**Distributed Transactions — The Hard Problem:** Di microservices, satu business operation melibatkan multiple services. Two-Phase Commit (2PC): sangat lambat dan blocking. Solusi modern: Saga Pattern. Setiap service menyelesaikan transaksi lokal dan mempublikasikan event. Jika ada yang gagal, jalankan "compensating transactions". Choreography (event-driven) vs Orchestration (saga orchestrator). Outbox Pattern: pastikan event dan DB write terjadi atomically (tulis event ke outbox table dalam transaksi yang sama, baru publish ke message broker via CDC atau polling).`,
      ...
      content_casual: `ACID menjamin keandalan transaksi: Atomicity (semua atau tidak sama sekali), Consistency (valid state ke valid state), Isolation (concurrent transactions tidak saling mempengaruhi), Durability (setelah commit, data persist meskipun crash).
...
**Distributed Transactions — The Hard Problem:** Di microservices, satu business operation melibatkan multiple services. Two-Phase Commit (2PC): sangat lambat dan blocking. Solusi modern: Saga Pattern. Setiap service menyelesaikan transaksi lokal dan mempublikasikan event. Jika ada yang gagal, jalankan "compensating transactions". Choreography (event-driven) vs Orchestration (saga orchestrator). Outbox Pattern: pastikan event dan DB write terjadi atomically (tulis event ke outbox table dalam transaksi yang sama, baru publish ke message broker via CDC atau polling).`,
      why_casual: `Pemahaman isolation levels mencegah subtle data integrity bugs: race conditions, lost updates, phantom reads. Di sistem dengan concurrent users tinggi (e-commerce, booking), pilihan isolation level yang tepat adalah perbedaan antara data konsisten dan data corrupt.`,
      mistake_casual: `Menggunakan Serializable untuk semua transaksi tanpa alasan — throughput database turun drastis. Tidak menangani deadlock — PostgreSQL otomatis abort satu transaksi, tapi aplikasi harus retry. Menggunakan 2PC di microservices modern — gunakan Saga + Outbox Pattern.`,
      interview: [
        {
...
3. **Exactly-once:** Paling susah dan mahal. Kafka mendukung ini lewat transactional API, tapi butuh setup ketat.`,
      ...
      content_casual: `Sistem monolitik memanggil service secara synchronous (REST/RPC). Kalau Service B mati, Service A error. Event-Driven Architecture memecahkan coupling ini menggunakan Message Broker (asynchronous).
...
2. **At-least-once:** Pesan dijamin sampai, tapi bisa di-deliver BERKALI-KALI jika ada retry/failure sebelum ACK. (Sistem harus Idempotent!).
3. **Exactly-once:** Paling susah dan mahal. Kafka mendukung ini lewat transactional API, tapi butuh setup ketat.`,
      why_casual: `Seni merancang arsitektur microservices adalah tentang mengelola komunikasi antar service. Mengerti kapan harus synchronous (gRPC) dan kapan harus asynchronous (Kafka/SQS) membedakan sistem yang rapuh (cascading failures) dari sistem yang tangguh.`,
      mistake_casual: `Menganggap At-least-once delivery berarti "semuanya aman". Di sistem terdistribusi, network request bisa timeout walau database udah terupdate, mengakibatkan broker mengirim pesan ulang. Jika consumer tidak dirancang **idempotent** (bisa menerima pesan yang sama berkali-kali tanpa efek samping ganda, misal: mengecek ID transaksi di DB sebelum memproses pembayaran), lo akan men-charge user 2 kali.`,
      interview: [
        {
